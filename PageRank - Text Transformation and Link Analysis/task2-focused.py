...
def store_graph():
	...
	f = open("Task2-FocusedGraph.txt", "w")
...
for key in graph:
	inlink_graph[key] = len(graph[key])

inlink_graph = sorted(inlink_graph.items(), key = operator.itemgetter(1))
inlink_graph.reverse() 

for i in range(25):
	print inlink_graph[i]